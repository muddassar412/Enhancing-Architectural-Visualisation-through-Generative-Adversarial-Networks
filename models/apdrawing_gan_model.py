...
class APDrawingGANModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        if self.isTrain:
            ...
            if self.opt.discriminator_local:
                self.netDLpillorl = networks.define_D(opt.input_nc + opt.output_nc, opt.ndf, 'multiscale',1,
                                          opt.n_layers_D, opt.norm, use_sigmoid, opt.init_type, opt.init_gain, self.gpu_ids)
                self.netDLpillorr = networks.define_D(opt.input_nc + opt.output_nc, opt.ndf, 'multiscale',1,
                                          opt.n_layers_D, opt.norm, use_sigmoid, opt.init_type, opt.init_gain, self.gpu_ids)
                self.netDLglass = networks.define_D(opt.input_nc + opt.output_nc, opt.ndf, 'multiscale',1,
                                          opt.n_layers_D, opt.norm, use_sigmoid, opt.init_type, opt.init_gain, self.gpu_ids)
                self.netDLbalconie = networks.define_D(opt.input_nc + opt.output_nc, opt.ndf, 'multiscale',1,
                                          opt.n_layers_D, opt.norm, use_sigmoid, opt.init_type, opt.init_gain, self.gpu_ids)
                self.netDLsky = networks.define_D(opt.input_nc + opt.output_nc, opt.ndf, 'multiscale',1,
                                          opt.n_layers_D, opt.norm, use_sigmoid, opt.init_type, opt.init_gain, self.gpu_ids)
                self.netDLwindows = networks.define_D(opt.input_nc + opt.output_nc, opt.ndf, 'multiscale',1,
                                          opt.n_layers_D, opt.norm, use_sigmoid, opt.init_type, opt.init_gain, self.gpu_ids)
                self.netDLroof = networks.define_D(opt.input_nc + opt.output_nc, opt.ndf, 'multiscale',1,
                                          opt.n_layers_D, opt.norm, use_sigmoid, opt.init_type, opt.init_gain, self.gpu_ids)
                self.netDLrailings = networks.define_D(opt.input_nc + opt.output_nc, opt.ndf, 'multiscale',1,
                                          opt.n_layers_D, opt.norm, use_sigmoid, opt.init_type, opt.init_gain, self.gpu_ids)
    ...
    def set_input(self, input):
        ...
        if self.opt.use_local:
            self.real_A_pillorl = input['pillorl_A'].to(self.device)
    ...
    def backward_D(self):
        ...
        if self.opt.discriminator_local:
            ...
            local_names = ['DLpillorl','DLpillorr','DLglass','DLbalconie','DLsky', ' DLwindows', ' DLrailings', 'DLroof','DLBG']
    ...
    def backward_G(self):
        ...
        ###local l1 vgg loss###
        if self.opt.use_local and not self.opt.no_G_local_loss:
            local_names = ['pillorl','pillorr','glass','balconie','sky', ' windows', ' railings', 'roof','BG']
