...
class SingleDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        if self.opt.use_local:
            ...
            center = torch.tensor([[feats[0,0],feats[0,1]-4*ratio],[feats[1,0],feats[1,1]-4*ratio],[feats[2,0],feats[2,1]-glass_H/2+16*ratio],[roof_x,roof_y]])
            item['center'] = center
            ...
            mask = torch.ones([output_nc,A.shape[1],A.shape[2]]) # mask out roof, pillar, glass, balconie
            for i in range(4):
            ...
            sky_A = (A/2+0.5) * mask.repeat(3,1,1) * mask2.repeat(3,1,1) * 2 - 1
